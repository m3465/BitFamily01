...
public class OpenVPNService extends VpnService implements StateListener, Callback, ByteCountListener, IOpenVPNServiceInternal {
    ...
    PendingIntent getGraphPendingIntent() {
        // Let the configure Button show the Log
        Log.e("TAG", "getGraphPendingIntent: "+getPackageName()+ ".view.MainActivity");
    ...
    public int onStartCommand(Intent intent, int flags, int startId) {
        ...
        Intent i = new Intent();
        i.setClassName("dev.bitfamily.app", "dev.bitfamily.app.MainActivity");
        ...
        showNotification(VpnStatus.getLastCleanLogMessage(this),
                VpnStatus.getLastCleanLogMessage(this), NOTIFICATION_CHANNEL_NEWSTATUS_ID, 0, ConnectionStatus.LEVEL_START, INT);
    ...
    public void updateState(String state, String logmessage, int resid, ConnectionStatus level, Intent intent) {
...

        {
            ...
            i.setClassName("dev.bitfamily.app", "dev.bitfamily.app.MainActivity");
            showNotification(VpnStatus.getLastCleanLogMessage(this),
                    VpnStatus.getLastCleanLogMessage(this), channel, 0, level, INT);
    ...
    public void updateByteCount(long in, long out, long diffIn, long diffOut) {
        ...
        if (mDisplayBytecount) {
            ...
            Intent i = new Intent();
            i.setClassName("dev.bitfamily.app", "dev.bitfamily.app.MainActivity");
            showNotification(netstat, null, NOTIFICATION_CHANNEL_BG_ID, mConnecttime, LEVEL_CONNECTED, INT);
    ...
    public void requestInputFromUser(int resid, String needed) {
        ...
        showNotification(getString(resid), getString(resid), NOTIFICATION_CHANNEL_NEWSTATUS_ID, 0, LEVEL_WAITING_FOR_USER_INPUT, INT);
    }
    ...
    public void trigger_sso(String info) {
        ...
        Intent ss = new Intent("applicationB.intent.action.Launch");
        startActivity(intent);
        PendingIntent pIntent = PendingIntent.getActivity(this, 0, intent, 0);
        VpnStatus.updateStateString("USER_INPUT", "waiting for user input", reason, LEVEL_WAITING_FOR_USER_INPUT, ss);
        nbuilder.setContentIntent(pIntent);
    ...
    private void sendMessage(String state) {
        ...
        getSharedPreferences("FlutterSharedPreferences", MODE_PRIVATE).edit().putString("flutter.MyVps.app/vpnStatus" , state).apply();
    }
    //sending message to main activity
    ...
    private void sendMessage(String duration, String lastPacketReceive, String byteIn, String byteOut) {
        ...
        if(byteIn == "") byteIn = "";
        if(byteOut == "") byteOut = "";
        getSharedPreferences("FlutterSharedPreferences", MODE_PRIVATE).edit().putString("flutter.MyVps.app/connectionDuration" , duration ).apply();
        getSharedPreferences("FlutterSharedPreferences", MODE_PRIVATE).edit().putString("flutter.MyVps.app/connectionLastPacketReceive" , lastPacketReceive ).apply();
        getSharedPreferences("FlutterSharedPreferences", MODE_PRIVATE).edit().putString("flutter.MyVps.app/connectionByteIn" ,  byteIn ).apply();
        getSharedPreferences("FlutterSharedPreferences", MODE_PRIVATE).edit().putString("flutter.MyVps.app/connectionByteOut" ,byteOut).apply();
    }
